...
import { calTokenAmountOut } from "../pumpFun/getAmountOut";
...
async function main() {
    ...
    const sourceTokenAccount = await PublicKey.createWithSeed(base, seed, TOKEN_PROGRAM_ID);
    ...
    const initAccountIx = createInitializeAccountInstruction(sourceTokenAccount, NATIVE_MINT, owner.publicKey);
    ...
    const swapArgs: any = {
        ...
        expectAmountOut: tokenAmountOut,
    ...
    const keys = [
        ...
        { pubkey: destinationTokenAccount, isWritable: true, isSigner: false }, // destination token account
